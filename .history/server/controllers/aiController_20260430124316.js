...
const assertAiAccess = async (req) => {
  ...
  if (!AI_ALLOWED_ROLES.has(req.user.role)) {
    throw new Error('AI access is not available for your account role');
  }
  if (req.user.status !== 'Approved') {
    throw new Error('Your account must be approved to use AI features');
  }
};
...
export const fetchModels = async (req, res) => {
  try {
    ...
    return serverError(res, error.message || "Failed to fetch AI models.");
  }
};
        ...
        const cleanHost = host.replace(/\/+$/, "");
          if (cleanHost.includes("5001") || cleanHost.includes("api/images")) {
            ...
            if (startRes.data.task_id) {
                ...
                try {
                  ...
                  if (task.state === 'SUCCESS') {
                    if (task.result && task.result.image_base64) {
                      ...
                      logger.info(`SD Image generated successfully after ${pollAttempts * 3}s`);
                      isFinished = true;
                    } else {
                    ...
                    logger.info(`SD task ${taskId} still ${task.state} (attempt ${pollAttempts + 1}/${MAX_POLL_ATTEMPTS})`);
                  }
                } catch (pollErr) {
                  if (pollErr.code === 'ECONNREFUSED' || pollErr.code === 'ENOTFOUND') {
                    throw new Error('Stable Diffusion Python service unreachable');
                  }
                  logger.warn(`Poll attempt ${pollAttempts + 1} failed:`, pollErr.message);
                }
              ...
              logger.info('SD Image generated immediately (no queue)');
            } else {
            ...
            logger.info('A1111 SD image generated');
          }
      }
